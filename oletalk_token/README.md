...
```bash
oletalk_token$ sam build --use-container
```
...
```bash
oletalk_token$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
oletalk_token$ sam logs -n HelloWorldFunction --stack-name oletalk_token --tail
```
...
```bash
oletalk_token$ pip install -r tests/requirements.txt --user
# unit test
oletalk_token$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
oletalk_token$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name oletalk_token
```
